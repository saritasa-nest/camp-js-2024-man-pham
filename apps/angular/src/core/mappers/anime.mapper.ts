...
export class AnimeMapper implements TMapper<AnimeDto, Anime> {
	...
	public fromDto(data: AnimeDto): Anime {
		return new Anime({
			...
			type: this.typeMapper.fromDto(data.type),
			status: this.statusMapper.fromDto(data.status),
			score: data.score,
	...
	public toDto(data: Anime): AnimeDto {
		return {
			...
			type: this.typeMapper.toDto(data.type),
			status: this.statusMapper.toDto(data.status),
			score: data.score,
